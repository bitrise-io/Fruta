...
let nutritionFacts: [String: NutritionFact] = {
    guard
        let jsonURL = Bundle.module.url(forResource: "NutritionalItems", withExtension: "json"),
        let jsonData = try? Data(contentsOf: jsonURL),
        let result = try? JSONDecoder().decode(Dictionary<String, NutritionFact>.self, from: jsonData)
    else {
        fatalError("Could not load NutritionFacts from NutritionalItems.json.")
    }
    return result
}()
